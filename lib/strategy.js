...
Strategy.prototype.userProfile = function (authCode, done) {
  this._oauth2._request('POST', this._oauth2._accessTokenUrl, {
    'Content-Type': 'application/json',
    'Accept': 'application/json'
  }, {
    auth_code: authCode
  }, this._oauth2.access_token, function (err, data, res) {
    if (err) throw err
    data = JSON.parse(data)
    if (data.user_info && data.user_info.userid) return done(null, data)
    throw new Error('obtain user profile failed.' + (data.errmsg || ''))
  })
}
  ...
  var _ensure = function () {
    oauthIns._request('GET', oauthIns._getAccessTokenUrl() + '?' + querystring.stringify(params), {
      ...
      if (data && data.provider_access_token) {
        ...
        throw new Error('obtain provider_access_token failed.' + (data.errmsg || ''))
      }
    })
  }
  _ensure()
